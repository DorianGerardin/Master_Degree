  ...
  for (int i=0; i < mean; i++){
   ...
   while (changementsDeClasse > min){
      ...
      for (int i=0; i < mean; i++){
        ...
        //printf("moyenne[%d] : %d %d %d" , i, moyenne[i][0],moyenne[i][1],moyenne[i][2]);
      }
   ...
   FILE *file;
    file = fopen("palette.dat", "w");
    for (int i=0; i < mean; i++)
    {
     fprintf(file, "%d %d %d\n",moyenne[i][0], moyenne[i][1], moyenne[i][2] );
    }

    fclose(file);
    int classes1d[nTaille];
    for (int i=0, j= 0; i < nTaille; i++, j+=3){
      classes1d[i] = classes[j];
    }
    for (int i=0; i < nTaille; i++){
      
      ImgOut[i] = classes1d[i];
    }
/*       if (classes[i] == 1){
